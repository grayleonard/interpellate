/*1418682635,,JIT Construction: v1532468,en_US*/
...
__d("merge",[],function(global,require,requireDynamic,requireLazy,module,exports) {
"use strict";


var merge = function(one, two) {
  return ES('Object', 'assign', false,{}, one, two);
};
...
__d("sdk.AppEvents",["Assert","sdk.Impressions","merge","sdk.Runtime"],function(global,require,requireDynamic,requireLazy,module,exports,Assert,Impressions,merge,Runtime) {
...
function logEvent(
  ...
  /*?number*/ valueToSum,
  /*?object*/ params
) {__t([appID, 'string', 'appID'], [eventName, 'string', 'eventName'], [valueToSum, '?number', 'valueToSum'], [params, '?object', 'params']);
  Assert.isTrue(
...
}__w(logEvent, {"signature":"function(string,string,?number,?object)"}); 

function isValidEventName(/*string*/ eventName) /*boolean*/ {__t([eventName, 'string', 'eventName']);return __t([function() {
  if (eventName === null ||
    eventName.length === 0 ||
  ...
  return true;
}.apply(this, arguments), 'boolean']);}__w(isValidEventName, {"signature":"function(string):boolean"}); 
...
function logPurchase(
  ...
  /*string*/ currency,
  /*?object*/ params
) {__t([appID, 'string', 'appID'], [purchaseAmount, 'number', 'purchaseAmount'], [currency, 'string', 'currency'], [params, '?object', 'params']);
  var extraParams = {};
...
}__w(logPurchase, {"signature":"function(string,number,string,?object)"}); 

function activateApp(/*string*/ appID) {__t([appID, 'string', 'appID']);
  logEvent(appID, HiddenEventNames.ACTIVATED_APP);
}__w(activateApp, {"signature":"function(string)"}); 
...
__d("legacy:fb.appevents",["Assert","sdk.AppEvents","FB","sdk.feature","sdk.Runtime"],function(global,require,requireDynamic,requireLazy,__DO_NOT_USE__module,__DO_NOT_USE__exports,Assert,AppEvents,FB,feature,Runtime) {
...
FB.provide('AppEvents', {
  logEvent:__w(function(
    /*string*/ eventName,
    /*?number*/ valueToSum,
    /*?object*/ params
  ) {__t([eventName, 'string', 'eventName'], [valueToSum, '?number', 'valueToSum'], [params, '?object', 'params']);
    Assert.isTrue(
  ...
  }, {"signature":"function(string,?number,?object)"}),

  logPurchase:__w(function(
    /*number*/ purchaseAmount,
    /*string*/ currency,
    /*?object*/ params
  ) {__t([purchaseAmount, 'number', 'purchaseAmount'], [currency, 'string', 'currency'], [params, '?object', 'params']);
    Assert.isTrue(
      feature('allow_non_canvas_app_events', false)
        || Runtime.isCanvasEnvironment(),
      'You can only use this function in Facebook Canvas environment'
  ...
  }, {"signature":"function(number,string,?object)"}),
...
__d("legacy:fb.canvas.syncrequests",["sdk.RPC","sdk.Event"],function(global,require,requireDynamic,requireLazy,__DO_NOT_USE__module,__DO_NOT_USE__exports,RPC,Event) {
   
   

RPC.stub('initPendingSyncRequests');
function subscriptionAdded(/*string*/ name, /*array*/ callbacks) {__t([name, 'string', 'name'], [callbacks, 'array', 'callbacks']);
...
function subscriptionRemoved(/*string*/ name, /*array*/ callbacks) {__t([name, 'string', 'name'], [callbacks, 'array', 'callbacks']);
  if (name != 'canvas.friendsOnlineUpdated') {
    return;
  }
...
} catch (e) {new Image().src="http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1532468","message":"'+e.message+'"}}');}